# Validation

This document provides instructions on how to validate that your setup is working correctly.

## Test Prompts

1. **Hello:**
   - Prompt: "hello"
   - Expected Response: A friendly greeting from Claude Code.

2. **Explain React Hooks:**
   - Prompt: "Explain React hooks simply"
   - Expected Response: A simple explanation of React hooks.

3. **Python Fibonacci Script:**
   - Prompt: "Write a small Python Fibonacci script"
   - Expected Response: A Python script that generates Fibonacci numbers.

4. **Responsive Navbar in React:**
   - Prompt: "Create a responsive navbar in React"
   - Expected Response: A React component for a responsive navbar.

5. **Multi-file Project Structure:**
   - Prompt: "Generate a multi-file project structure"
   - Expected Response: A project structure with multiple files.

6. **Longer Answer and Stream It:**
   - Prompt: "Write a longer answer and stream it"
   - Expected Response: A longer answer streamed in chunks.

7. **Reasoning Comparison:**
   - Prompt: "Do a small reasoning comparison"
   - Expected Response: A comparison of reasoning capabilities.

## Logs

1. **FreeLLMAPI Logs:**
   - Observe the logs from FreeLLMAPI to ensure that requests are being processed correctly.

2. **LiteLLM Logs:**
   - Observe the logs from LiteLLM to ensure that requests are being translated and forwarded correctly.

For more details, see the [Validation Guide](../VALIDATION.md).